...
contract MultiSig is Ownable {
    ...
    event Executed(uint txId);
     ...
     function approve(uint _txId) external onlyOwners
           txExists(_txId)
           notApproved(_txId)
           txExists(_txId) {
     ...
     function revoke(uint _txId) external onlyOwners
           txExists(_txId)
           notApproved(_txId)
           notExecuted(_txId)
           wasApproved(_txId) {
           ...
           modifier enoughtApprovals(uint _txId) {
               require(approvalsCount[_txId] >= requiredApprovals, "not enought approvals");
               _;
           }
